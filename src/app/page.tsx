...
function IssueList({ title, issues, className }: { title: string; issues: ValidationIssue[]; className: string }) {
  return (
    <div className={`validationBlock ${className}`}>
      <h3>{title}</h3>
      {issues.length ? (
        <ul>{issues.map((issue, index) => <li key={`${issue.level}-${issue.scope}-${issue.sourceRow}-${issue.field}-${index}`}>{issue.sourceRow ? `${issue.sourceRow}行目: ` : ''}[{issue.scope}] {issue.field}: {issue.message}</li>)}</ul>
      ) : <p>ありません。</p>}
    </div>
  );
}
...
function ExcelPreview({ result, selectedSequence, onSelectApplicant }: { result: BatchParseResult; selectedSequence: number | null; onSelectApplicant: (applicant: BatchApplicant) => void }) {
  ...
  return (
    <section className="excelPreview">
      ...
      <p className="selectedApplicant">現在選択中の申請人: {selectedApplicant ? `${selectedApplicant.sequence}. ${selectedApplicant.passportName}` : '（未選択）'}</p>
      ...
      <section className="previewGroup">
        ...
        <div className="tableWrap">
          <table>
            <thead><tr><th>選択</th><th>連番</th><th>Excel行</th><th>氏名</th><th>国籍</th><th>職業</th><th>性別</th><th>生年月日</th><th>年齢</th><th>状態</th></tr></thead>
            <tbody>
              {result.reviewApplicants.map((row) => {
                ...
                return (
                  <tr key={`${row.sourceRow}-${row.sequence}`} className={`${hasError ? 'rowError' : ''} ${isSelected ? 'rowSelected' : ''}`.trim()} onClick={() => applicant && onSelectApplicant(applicant)}>
                    ...
                    <td>{row.sequence}</td><td>{row.sourceRow}</td><td>{row.passportName || '（未入力）'}</td><td>{row.nationality || '（未入力）'}</td><td>{row.occupation || '（未入力）'}</td><td>{row.gender || '（未入力）'}</td><td>{row.dateOfBirth || '（未入力）'}</td><td>{row.calculatedAge ?? '（計算不可）'}</td><td>{row.status === 'valid' ? '有効' : row.errors.join('／')}</td>
                  </tr>
                );
              })}
            </tbody>
...
export default function Home() {
  ...
  async function regeneratePreview(debugOverride = isDebug) {
    ...
    try {
      ...
      await renderInvitationCanvas(selected?.data ?? fixedInvitationReasonSample, { canvas: previewCanvasRef.current, debug: debugOverride });
  ...
  async function downloadCompletedPng() {
    ...
    try {
      const selected = getSelectedUploadedData();
      const canvas = await renderInvitationCanvas(selected?.data ?? fixedInvitationReasonSample, { debug: false });
  ...
  async function downloadSelectedApplicantPdf() {
    ...
    try {
      const canvas = await renderInvitationCanvas(selected.data, { debug: false });
      const blob = await exportInvitationPdf(canvas);
  ...
  async function downloadAllApplicantsZip() {
    ...
    try {
      ...
      for (const [index, applicant] of excelResult.applicants.entries()) {
        ...
        const data = toInvitationReasonData(common, applicant);
        const canvas = await renderInvitationCanvas(data, { debug: false });
        const blob = await exportInvitationPdf(canvas);
        entries.push({ filename: createInvitationReasonPdfFilename(applicant), blob });
      }
      setRenderMessage('ZIPファイルを作成しています...');
      const zipBlob = await createZipBlob(entries);
      downloadBlob(zipBlob, 'InvitationReason_AllApplicants.zip');
  ...
  async function handleExcelUpload(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setExcelResult({ fileName: file.name, common: {}, reviewApplicants: [], applicants: [], loadedApplicantRows: 0, validApplicantCount: 0, canGenerateBatch: false, issues: [{ level: 'error', scope: 'common', field: 'file', message: caughtError instanceof Error ? caughtError.message : 'Excelファイルの読み込みに失敗しました。' }] });
  ...
  return (
    <main>
      <section className="hero">
        ...
        <h1>複数申請人Excel取込とPDF作成</h1>
        <p>Excelから申請人を選択して1名分を作成するほか、有効な申請人全員分の個別PDFをZIPで一括ダウンロードできます。</p>
      </section>
      <section className="rendererSection">
        <p className="eyebrow">Canvas PDF Renderer</p>
        <h2>選択申請人プレビュー</h2>
        <p>有効な申請人が選択されている場合はそのデータを描画し、それ以外は固定サンプルを表示します。</p>
        <div className="buttonRow">
          ...
          <button type="button" onClick={downloadCompletedPdf} disabled={isRendering}>固定サンプルPDFをダウンロード</button>
          <button type="button" onClick={downloadSelectedApplicantPdf} disabled={isRendering || !selectedReady}>選択中の1名をPDFでダウンロード</button>
          <button type="button" onClick={downloadAllApplicantsZip} disabled={isRendering || !batchReady}>有効な申請人全員をZIPでダウンロード</button>
        </div>
      ...
      <section className="excelSection">
        ...
        <p>「共通情報」と「申請人一覧」の2シートを読み込みます。無効行は赤色で残り、一括生成はすべての行が有効な場合のみ実行できます。</p>
        <div className="buttonRow">
        ...
        {excelResult ? <ExcelPreview result={excelResult} selectedSequence={selectedSequence} onSelectApplicant={(applicant) => setSelectedSequence(applicant.sequence)} /> : null}
      </section>
    </main>
  );
}
