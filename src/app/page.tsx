...
function ExcelPreview({ result, selectedSequence, onSelectApplicant }: { result: BatchParseResult; selectedSequence: number | null; onSelectApplicant: (applicant: BatchApplicant) => void }) {
  ...
  return (
    <section className="excelPreview">
      ...
      <section className="previewGroup"><h3>申請人一覧</h3><div className="tableWrap"><table><thead><tr><th>選択</th><th>連番</th><th>Excel行</th><th>氏名</th><th>国籍</th><th>職業</th><th>性別</th><th>生年月日</th><th>年齢</th><th>状態</th></tr></thead><tbody>
...
export default function Home() {
  ...
  async function downloadCompletedPng() {
    ...
    try { const selected = getSelectedUploadedData(); const canvas = await renderInvitationCanvas(selected?.data ?? fixedInvitationReasonSample, { debug: false }); const blob = await canvasToPngBlob(canvas); downloadBlob(blob, `${invitationReasonDownloadBaseName}.png`); setRenderMessage('完成PNGを作成しました。'); }
  ...
  async function downloadSelectedApplicantPdf() {
    ...
    try { const canvas = await renderInvitationCanvas(selected.data, { debug: false }); const blob = await exportInvitationPdf(canvas); downloadBlob(blob, createInvitationReasonPdfFilename(selected.applicant)); setRenderMessage('選択中の申請人PDFを作成しました。'); }
  ...
  return <main>
    <section className="hero"><p className="eyebrow">招へい理由書 作成ツール</p><h1>複数申請人Excel取込と個別PDF作成</h1><p>Phase 2では、Excelから有効な申請人を選択し、Canvasプレビューと1名分の個別PDFへ接続します。ZIP一括生成はまだ行いません。</p></section>
    <section className="rendererSection"><p className="eyebrow">Canvas PDF Renderer</p><h2>選択申請人プレビュー</h2><p>有効な申請人が選択されている場合はそのデータを描画し、それ以外は固定サンプルを表示します。</p><div className="buttonRow"><button type="button" onClick={() => regeneratePreview(isDebug)} disabled={isRendering}>{isRendering ? '処理中...' : 'プレビューを再生成'}</button><button type="button" onClick={downloadCompletedPng} disabled={isRendering}>完成PNGをダウンロード</button><button type="button" onClick={downloadCompletedPdf} disabled={isRendering}>固定サンプルPDFをダウンロード</button><button type="button" onClick={downloadSelectedApplicantPdf} disabled={isRendering || !selectedReady}>選択中の1名をPDFでダウンロード</button></div><div className="canvasFrame"><canvas ref={previewCanvasRef} width={2481} height={3508} aria-label="招へい理由書Canvasプレビュー" /></div><div className="result" aria-live="polite"><h3>レンダリング状況</h3><p>{renderMessage}</p>{renderError ? <p className="error">{renderError}</p> : null}</div></section>
    <section className="excelSection"><p className="eyebrow">Excel入力（Phase 2）</p><h2>複数人用Excelテンプレートのダウンロード・アップロード</h2><p>「共通情報」と「申請人一覧」の2シートを読み込み、有効な申請人を選択できます。無効行は赤色で残りますが選択できません。</p><div className="buttonRow"><button type="button" onClick={downloadExcelTemplate}>複数人用Excelテンプレートをダウンロード</button><label className="uploadButton">Excelファイルをアップロード<input type="file" accept=".xlsx,.xls" onChange={handleExcelUpload} /></label></div>{excelResult ? <ExcelPreview result={excelResult} selectedSequence={selectedSequence} onSelectApplicant={(applicant) => setSelectedSequence(applicant.sequence)} /> : null}</section>
  </main>;
}
