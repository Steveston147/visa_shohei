...
import {
  invitationReasonPdfFields,
  type InvitationReasonPdfFieldKey,
} from "../lib/pdfFieldNames";
import { renderInvitationReasonPdf } from "../lib/invitationPdfRenderer";
...
const excelTemplateRows: TemplateRow[] = [
  {
    section: "Program",
    field: "programName",
    value: "Ritsumeikan Short-term Study Program",
    note: "管理用。PDFには表示しない",
  },
  {
    section: "Program",
    field: "documentDate",
    value: "2026-07-08",
    note: "YYYY-MM-DD。文字列として扱う",
  },
  {
    section: "Program",
    field: "diplomaticMission",
    value: "上海",
    note: "PDFの「在」と「日本国」の間に入る文字。例：上海",
  },
  {
    section: "Inviter",
    field: "postalCode",
  ...
  { section: "Inviter", field: "extension", value: "511-4794", note: "内線" },
  {
    ...
    value: "Ritsumeikan Study Abroad Center",
    note: "担当者所属先名",
  },
  {
    section: "Organisation",
    field: "contactPersonName",
    value: "Takeshi Tanaka",
    note: "担当者氏名",
  },
  {
    section: "Organisation",
  ...
  },
  {
    section: "Invitation",
    field: "purpose",
    value: "短期留学生受入プログラムに参加するため",
    note: "招へい目的",
  },
  {
    section: "Invitation",
    field: "background",
    value: "協定校から推薦された学生を受け入れるため",
    note: "招へい経緯",
  },
  {
    section: "Invitation",
...
const excelFieldSet = new Set(
  excelTemplateRows.map((row) => `${row.section}|${row.field}`),
);
const dateFieldSet = new Set(["Program|documentDate", "Applicant|dateOfBirth"]);
const optionalExcelFields = new Set([
  "Inviter|extension",
  "Organisation|extension",
]);
const requiredExcelFields = excelTemplateRows.filter(
  (row) => !optionalExcelFields.has(`${row.section}|${row.field}`),
);
...
function assignExcelValue(
  data: ParsedExcelData,
  section: string,
  field: string,
  value: string,
) {
  if (section === "Program" && field in data.program)
    data.program[field as keyof ProgramInfo] = value;
  if (section === "Inviter" && field in data.inviter)
    data.inviter[field as keyof InviterInfo] = value;
  if (section === "Organisation" && field in data.organisation)
    data.organisation[field as keyof OrganisationContact] = value;
  if (section === "Applicant" && field in data.applicant)
    data.applicant[field as keyof Applicant] = value;
  if (section === "Invitation" && field in data.invitation)
    data.invitation[field as keyof InvitationInfo] = value;
}
...
function getExcelValue(data: ParsedExcelData, section: string, field: string) {
  ...
  if (section === "Organisation")
    return data.organisation[field as keyof OrganisationContact];
  if (section === "Applicant") return data.applicant[field as keyof Applicant];
  if (section === "Invitation")
    return data.invitation[field as keyof InvitationInfo];
  return "";
}

function validateExcelData(data: ParsedExcelData, errors: string[]) {
  for (const row of requiredExcelFields) {
    if (!getExcelValue(data, row.section, row.field))
      errors.push(`必須項目が未入力です: ${row.section} / ${row.field}`);
  }
  if (
    data.program.documentDate &&
    !/^\d{4}-\d{2}-\d{2}$/.test(data.program.documentDate)
  )
    errors.push("documentDate は YYYY-MM-DD 形式で入力してください。");
  if (
    data.applicant.dateOfBirth &&
    !/^\d{4}-\d{2}-\d{2}$/.test(data.applicant.dateOfBirth)
  )
    errors.push("dateOfBirth は YYYY-MM-DD 形式で入力してください。");
  if (
    data.applicant.gender &&
    !["male", "female"].includes(data.applicant.gender)
  )
    errors.push("gender は male または female を入力してください。");
  if (
    data.applicant.passportName &&
    !/^[A-Za-z '\-]+$/.test(data.applicant.passportName)
  )
    errors.push(
      "passportName は英字、スペース、ハイフン、アポストロフィのみで入力してください。",
    );
  if (data.inviter.postalCode && !/^\d{3}-\d{4}$/.test(data.inviter.postalCode))
    errors.push("postalCode は 3桁-4桁（例: 603-8577）で入力してください。");
}
...
function toReiwaYear(date: Date) {
  ...
  if (date.getTime() < reiwaStart)
    throw new Error("令和より前の日付はこのサンプルでは対応していません。");
...
const sampleValues: Record<InvitationReasonPdfFieldKey, string> = {
  ...
  applicantGenderFemale:
    sampleData.gender === "female" ? "checked" : "unchecked",
...
const mappingRows = (
  Object.keys(requiredFieldLabels) as InvitationReasonPdfFieldKey[]
).map((key) => ({
  key,
  label: requiredFieldLabels[key],
  fieldName: invitationReasonPdfFields[key],
  sampleValue: sampleValues[key],
  status: invitationReasonPdfFields[key] ? "mapped" : "unmapped",
}));
...
function ExcelPreview({ result }: { result: ExcelParseResult }) {
  ...
  return (
    <section className="excelPreview">
      ...
      />
      <div className="validationBlock errors">
      ...
      <div className="validationBlock warnings">
...
export default function Home() {
  const [message, setMessage] = useState(
    "下のボタンから固定サンプルデータの招へい理由書PDFを作成できます。",
  );
  ...
  async function downloadExcelTemplate() {
    ...
    const worksheet = XLSX.utils.json_to_sheet(excelTemplateRows, {
      header: ["section", "field", "value", "note"],
    });
    ...
    try {
      ...
      if (!worksheet) errors.push("Input シートが見つかりません。");
        ...
        rows.forEach((row, index) => {
          ...
          if (!section && !field && !stringifyExcelValue(rawValue) && !note)
            return;
          const key = `${section}|${field}`;
          if (!excelFieldSet.has(key)) {
            warnings.push(
              `未定義の項目です（${index + 2}行目）: ${section || "section空欄"} / ${field || "field空欄"}`,
            );
            return;
          }
          const value = dateFieldSet.has(key)
            ? normalizeExcelDate(rawValue, XLSX)
            : stringifyExcelValue(rawValue);
          if (dateFieldSet.has(key) && value === null) {
            errors.push(
              `${section} / ${field} は YYYY-MM-DD に変換できません。入力値を確認してください。`,
            );
            return;
          }
  ...
  async function generatePdf() {
    ...
    try {
      ...
      if (unmappedRows.length > 0) {
        throw new Error(
          `PDFフィールド未設定: ${unmappedRows.map((row) => row.label).join("、")}。docs/pdf-field-inspection.md の座標で確認してから設定してください。`,
        );
      }

      const reiwaYear = toReiwaYear(documentDate);
      const bytes = await renderInvitationReasonPdf({
        documentDateYear: String(reiwaYear),
        documentDateMonth: String(documentDate.getUTCMonth() + 1),
        documentDateDay: String(documentDate.getUTCDate()),
        diplomaticMission: sampleData.diplomaticMission,
        inviterPostalCodeFirst3: postalFirst3,
        ...
        applicantPassportName: sampleData.passportName,
        applicantGenderMale:
          sampleData.gender === "male" ? "checked" : "unchecked",
        applicantGenderFemale:
          sampleData.gender === "female" ? "checked" : "unchecked",
        additionalApplicantsCount: "",
        applicantDateOfBirthYear: String(birthDate.getUTCFullYear()),
      ...
      setMessage(
        "招へい理由書PDFを作成しました。ダウンロードをご確認ください。",
      );
    } catch (caughtError) {
      const errorMessage =
        caughtError instanceof Error
  ...
  return (
    <main>
      <section className="hero">
        ...
        <p>
          申請人1名分のサンプルデータを使用し、PDFテンプレートへ入力してフラット化したPDFをダウンロードします。
        </p>
        <button type="button" onClick={generatePdf} disabled={isLoading}>
          {isLoading ? "作成中..." : "完成PDFをダウンロード"}
        </button>
      </section>
      ...
      <section className="excelSection">
        ...
        <p>
          Excelで入力した内容を画面上で確認します。この段階ではアップロード内容をPDF生成には接続しません。
        </p>
      ...
      <section className="debug">
        ...
        <p>
          AcroFormの順番だけで判断せず、
          <code>docs/pdf-field-inspection.md</code> の座標で確認した対応だけを
          mapped にします。
        </p>
            ...
            <tbody>
              {mappingRows.map((row) => (
                <tr key={row.key}>
                  ...
                  <td
                    className={row.status === "mapped" ? "mapped" : "unmapped"}
                  >
                    {row.status}
                  </td>
                </tr>
